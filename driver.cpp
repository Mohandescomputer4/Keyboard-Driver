...
// Splits a given string by '+' and returns a vector of substrings.
// - keyname : is string include in keys which are pressed
vector<string> splitString(string keyname) {
    ...
    string::size_type end = keyname.find('+');

    // Loop to find each '+' and extract the substring before it
    while (end != string::npos) {
        result.push_back(keyname.substr(start, end - start));
        start = end + 1;
        end = keyname.find('+', start);
    }

    // Add the last substring after the last '+'
    result.push_back(keyname.substr(start));
    return result;
}
